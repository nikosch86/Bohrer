...
deps:
	...
	docker run --rm -v $(PWD):/app -w /app golang:1.21-alpine go mod download

build:
	docker compose build

test:
	docker run --rm -v $(PWD):/app -w /app -e CGO_ENABLED=1 golang:1.21 go test -v -race -coverprofile=coverage.out ./...
	docker run --rm -v $(PWD):/app -w /app golang:1.21-alpine go tool cover -html=coverage.out -o coverage.html
